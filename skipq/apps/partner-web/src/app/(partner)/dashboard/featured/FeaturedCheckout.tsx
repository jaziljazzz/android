...
const TIERS: Tier[] = [
  { weeks: 1, priceInr: 999, label: "1 week", hint: "Try it out" },
  {
    weeks: 4,
    priceInr: 2499,
    label: "4 weeks",
    hint: "Best value for the month",
    recommended: true,
  },
  { weeks: 12, priceInr: 5999, label: "12 weeks", hint: "Stay on top all quarter" },
];
...
export function FeaturedCheckout({ salonName }: { salonName: string }) {
  ...
  useEffect(() => {
    loadCheckout().catch(() => {
      // Soft-fail; user can still try the button which will retry.
    });
  }, []);

  async function buy(weeks: number) {
    setError(null);
    setSuccess(null);
    ...
    try {
      ...
      const res = await fetch(`${supabaseUrl}/functions/v1/create-featured-order`, {
        method: "POST",
        headers: {
      ...
      const rzp = new window.Razorpay!({
        ...
        name: "SkipQ",
        description: `Featured listing · ${weeks} week${weeks === 1 ? "" : "s"}`,
        prefill: { email: userEmail },
        theme: { color: "#FF5454" },
        handler: () => {
          setSuccess(
            "Payment received. Your featured listing activates as soon as Razorpay confirms (usually under 30 seconds).",
          );
  ...
  return (
    <section className="mt-6">
      <h2 className="text-sm font-bold uppercase tracking-wider text-skip-stone">
        Pick a package — {salonName}
      </h2>
      <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-4">
        {TIERS.map((t) => {
          const busy = pendingWeeks === t.weeks;
          return (
            <div
              key={t.weeks}
              className={`skip-card p-5 flex flex-col gap-3 ${
            ...
            >
              ...
              <button
                type="button"
                onClick={() => buy(t.weeks)}
                disabled={pendingWeeks !== null}
                className={`mt-auto ${t.recommended ? "skip-btn-primary" : "skip-btn-secondary"} ${
                  busy ? "opacity-60" : ""
                }`}
              >
      ...
      <p className="mt-6 text-xs text-skip-stone">
        Payments are processed by Razorpay. Featured time stacks on top of any existing
        window — buying 4 weeks while 5 days remain leaves you with 33 days total.
      </p>
    </section>
  );
}
